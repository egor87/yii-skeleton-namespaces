...
abstract class BaseModel extends \CActiveRecord
{
	...
	/**
	 * Получить форматированную дату из указанного атрибута
	 * @param type $attr
	 */
	protected function _getDateF($attr, $showTime = TRUE)
	{
		if (!$this->hasAttribute($attr) || is_null($this->$attr) && $this->$attr == '')
		{
			return '';
		}
		if (preg_match('/^[0-9]{4}\-[0-9]{2}\-[0-9]{2}$/', $this->$attr))
		{
			return \DateTime::createFromFormat('Y-m-d', $this->$attr)->format('d.m.Y');
		}
		elseif (preg_match('/^[0-9]{4}\-[0-9]{2}\-[0-9]{2}\s[0-9]{2}\:[0-9]{2}\:[0-9]{2}$/', $this->$attr))
		{
			$date = \DateTime::createFromFormat('Y-m-d H:i:s', $this->$attr);
			if ($showTime)
			{
