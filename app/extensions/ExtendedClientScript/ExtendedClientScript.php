				...
				*/
				...
				foreach ($this->cssFiles as $url => $media)
				{
					...
					if (!empty($this->excludeCssFiles) &&
						in_array($file_arr['basename'], $this->excludeCssFiles)
						|| $this->isRemoteFile($url))
					{
						continue;
					}
					else
			...
			if (isset($this->scriptFiles[$pos]) && count($this->scriptFiles[$pos]) !==  0)
			{
				...
				*/
				...
				foreach ($jsFiles as &$fileName)
				{
					...
					(!empty($this->excludeJsFiles) && in_array($file_arr['basename'], $this->excludeJsFiles) || $this->isRemoteFile($fileName)) AND $fileName = false;
				}
